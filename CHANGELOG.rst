...
* [Fix] Corrected the Glyphicon compatibility layer, where 59 of 96 mappings pointed at
  the wrong Bootstrap Icons glyph. Most visibly ``.icon-calendar`` -- used by
  ``SuitDateWidget`` -- rendered ``bi-calendar-x-fill``, putting an X on every date field
  in the admin. Bootstrap Icons assigns codepoints alphabetically, so an upstream bump
  shifts every codepoint after the inserted icon and a hand-written table of ``\f1f4``
  literals silently rots.
* [Feature] ``scss/_icons.scss`` is now generated by ``npm run build:icons`` from a
  name-keyed map in ``scripts/gen-icons-scss.js``, resolving codepoints from the
  installed ``bootstrap-icons`` package at build time. Unknown icon names and
  duplicate legacy classes fail the build instead of producing a wrong glyph.
* [Fix] ``.date-icon`` in ``ui/_form.scss`` carried the same stale codepoint, so Django's
  own date shortcut icon drew ``bi-calendar-x-fill`` too.
* [Fix] The date/time widgets no longer hide Django's ``.datetimeshortcuts``. Hiding the
  span removed the "Today"/"Now" links and also misplaced the picker: ``DateTimeShortcuts``
  positions ``calendarbox``/``clockbox`` from the icon link's ``findPosX``/``findPosY``, and a
  ``display: none`` link measures ``(0, 0)``, so the popup opened at the top of the page.
  The icon link now stays in the layout and is styled as the widget's button, with the
  decorative ``.add-on`` shown only until Django replaces it.
* [Fix] The date/time button is flush with the input again and matches its height. Both it
  and ``.add-on`` now stretch to the input, and the shortcuts span is no longer reordered --
  reordering jumped it past anything a project appends to the widget, which detached the
  button from the input and left the appended control sitting between the two.
* [Fix] Calendar day numbers are centered again. Django builds the popup as
  ``class="calendarbox module"``, so the dashboard defaults in ``ui/_actionlist.scss``
  (``.module table th, td``) outranked ``.calendar td`` and left-aligned the grid, re-padded
  its cells and drew an extra border under every row. Those defaults now skip
  ``.calendarbox``/``.clockbox``, which also retires the ``!important`` padding in
  ``ui/_calendar.scss`` that had been added to fight the same collision. The exclusion uses
  ``:where()`` so it adds no specificity -- a bare ``:not()`` chain would make the rule strong
  enough to win fights it previously lost, starting with the changelist (also ``.module``),
  whose data header it would repaint light while the header text stayed white.
* [Fix] ``<select multiple>`` filters in the changelist search bar size themselves again.
  ``#changelist-search .search-container select`` pins ``height: 30px`` for the single-select
  filters, and at (1,1,1) it outranked the ``select[multiple] { height: auto }`` rule meant to
  exempt them -- a multi-select with ``size="8"`` collapsed to roughly one row and lost its
  width to ``width: 100%`` of a shrink-wrapping wrapper.
* [Fix] The search bar's "Keyword" label is visually hidden -- the input already carries the
  same word as its placeholder. It stays in the markup for screen readers and the for/id
  association.
* [Fix] Integer inputs take the shared ``--suit-control-width`` (220px) instead of a
  hardcoded 80px. Django renders ``IntegerField`` as ``<input type="number" class="vIntegerField">``,
  so the 80px rule and the themed ``input[type="number"]`` rule matched the same element at
  identical specificity and the winner came down to source order -- the token-driven rule was
  dead. Narrowing these fields again should raise specificity deliberately, not rely on
  which rule happens to come last.
* [Feature] Completed the Glyphicon compatibility map: it covered the 96 names django-suit's
  own templates used, leaving the rest of Bootstrap 2's set (``icon-bell``, ``icon-wrench``,
  ``icon-flag``, ``icon-briefcase``, ...) unmapped. An unmapped ``.icon-*`` is invisible rather
  than broken -- it still picks up the bootstrap-icons font and simply has no glyph -- so
  projects carrying 0.2.x markup lost icons silently. Now 154 legacy classes.
* [Feature] Added a Bootstrap 2/3 compatibility shim for ``.close`` (``ui/_compat.scss``).
  Bootstrap 5 renamed the dismiss control to ``.btn-close``, so leftover ``.close`` markup --
  react-bootstrap < 1.0 still emits it, as do templates carried over from django-suit 0.2.x --
  rendered as a bare browser button. It is also placed with both ``float`` and flex ``order``,
  since Bootstrap 5 made ``.modal-header`` a flex container where the original float is ignored
  and the button drifts to whatever position it holds in the DOM.
* [Tests] Added ``suit5.tests.icons``, which fails if any codepoint disagrees with
  ``bootstrap-icons.json`` -- anywhere in the SCSS tree, not just the generated map -- if a
  legacy class is mapped twice, or if the committed ``suit.css`` is stale relative to the SCSS.
